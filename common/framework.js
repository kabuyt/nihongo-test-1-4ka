...
TF.submitResults = async function() {
  ...
  const studentIdEl = document.getElementById('student_id');
  const studentId = studentIdEl ? studentIdEl.value.trim().toUpperCase() : '';
  if (!studentId) { alert('学生ID（Student ID）を入力してください。\n例：VJC-001'); return; }
  ...
  try {
    ...
    if (!trainees || trainees.length === 0) {
      ...
      status.textContent = '❌ 学生ID「' + studentId + '」が見つかりません。先生に確認してください。';
      return;
    }
    ...
    if (!insertRes.ok) throw new Error('登録失敗 (' + insertRes.status + ')');

    // GASにも送信（設定されている場合）
    if (_gasUrl && _gasUrl !== 'YOUR_GAS_URL_HERE') {
      let freeAnswers = {};
      if (typeof window.collectFreeAnswers === 'function') freeAnswers = window.collectFreeAnswers();
      const payload = {
      ...
      fetch(_gasUrl, { method:'POST', mode:'no-cors', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) });
    }
