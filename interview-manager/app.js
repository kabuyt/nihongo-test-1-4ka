...
const TEST_GUIDE = {
  kraepelin: {
    ...
    note: '評価は「作業量45点（解いた量）」「正確性35点（誤答の少なさ・誤答20%以上は0点）」「安定性20点（作業量の推移が標準的な形にどれだけ近いか）」の合計100点です。作業量はその面接で最も多く解いた方を満点とした相対評価です。',
  },
  math: {
    title: '数学テスト',
          ...
          return `
            <tr>
              ...
              ${isTestEnabled(interview, 'kraepelin') ? `<td class="print-kraepelin">${row.kraepelinEval ? `<strong>${formatScore(row.kraepelinEval.total)}点</strong>
                ...
                <span class="print-kraepelin-breakdown"><b>正確性</b><em>${row.kraepelinEval.accuracy}<i>/35</i></em></span>
                <span class="print-kraepelin-breakdown"><b>安定性</b><em>${row.kraepelinEval.stability}<i>/20</i></em></span>
  ...
  body.innerHTML = rows.map(row => {
    ...
    const kraepelinResult = row.kraepelinEval == null
      ...
      : `<span class="status-pill ok">${row.kraepelinEval.total}</span>
         ...
         <div class="mini">作業量 ${row.kraepelinEval.work}・正確性 ${row.kraepelinEval.accuracy}・安定性 ${row.kraepelinEval.stability} / ${judgmentLabel(row.kSummary.judgment)}</div>
