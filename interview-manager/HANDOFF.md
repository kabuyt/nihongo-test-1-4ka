...
- 数学テストはQR受験の入力式アプリ（`math-test/`）で自動採点し、点を自動取り込み（ベトナム国語と同じ保存経路）。数学検定5級第1回の30問。回答は4択ではなく、数字入力／分数□÷□／比□:□／一次式□x+□の枠入力、図形2問のみプルダウン。約分違い・全角数字も正解判定。図つき3問は画像を assets/ に同梱。紙受験＋管理表で点を手入力する運用も可（点欄は残してある）
- 総合順位をWeb上で表示。各科目を100点満点に揃えた得点の合計で決定（クレペリン=独自100点、数学=自動採点100点、ベトナム国語=100点満点、日本語単語=30問を100換算、ピンボード=下記）
- ピンボードは翌日実施が多いため、未入力の科目を除いた「暫定順位」を常時表示。全員に入っている科目だけの合計で順位付けし、「仮 第N位／270 / 300点（3/4科目）」の形で表示。全科目そろうと自動で確定順位（第N位）に切替。1人でも全科目空なら「未集計」
